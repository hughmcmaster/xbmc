...
#include "cores/VideoPlayer/DVDSubtitles/DVBTeletextParser.h"
...
bool CDVDOverlayCodecTeletext::Open(CDVDStreamInfo& hints, CDVDCodecOptions& options)
{
  ...
  m_pOverlay.reset();
  m_order = 0;
  m_parser = hints.teletextParser;

  if (!m_parser)
  {
    CLog::Log(LOGERROR,
              "CDVDOverlayCodecTeletext::Open: shared teletext parser unavailable");
    return false;
  }
...
OverlayMessage CDVDOverlayCodecTeletext::Decode(DemuxPacket* pPacket)
{
  ...
  const auto pages = m_parser->GetSubtitlePages();
  bool added = false;

  for (const auto& page : pages)
  {
    std::vector<uint8_t> rows;
    DVBTeletextPage metadata;
    if (!m_parser->GetPageRows(page.page, page.subPage, rows, metadata))
      continue;

    const std::string assText = ConvertTeletextRowsToASS(rows);
    CLog::Log(LOGINFO, "{:d} assText: {}", rows.size(), assText);
    if (assText.empty())
      continue;

    const int ret = m_libass->AddEvent(assText.c_str(), pts, pts + duration, nullptr);
    if (ret != ASS_NO_ID)
      added = true;
  }
...
std::string CDVDOverlayCodecTeletext::ConvertTeletextRowsToASS(const std::vector<uint8_t>& rows)
{
  if (rows.empty())
    return "";
